...
function makePreview(item, interactive = false) {
...
export async function pickLora() {
    ...
    return new Promise((resolve) => {
        ...
        bg.innerHTML = `<div class="fla-lp" role="dialog" aria-modal="true"><div class="fla-lp-head"><input type="search" placeholder="${t("searchLora")}"><button class="fav">☆ ${t("loraFavorites")}</button><button class="close" title="${t("cancel")}">×</button></div><div class="fla-lp-main"><aside class="fla-lp-side"></aside><section class="fla-lp-content"><div class="fla-lp-path"></div><div class="fla-lp-grid"></div></section></div></div>`;
        ...
        function renderSide() {
            ...
            descendants.onclick = () => { includeChildren = !includeChildren; render(); };
        ...
        input.addEventListener("input", render); fav.onclick = () => { favoritesOnly = !favoritesOnly; render(); }; bg.querySelector(".close").onclick = () => close(); bg.onclick = (event) => { if (event.target === bg) close(); };
